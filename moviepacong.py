...
for x in range(len(movie)):
	if float(pingfeng[x]) >= 7:
		str+= '<html><body><a href="'+links[x]+'" target="_blank">'+movie[x]+'\t'+'<font color="#FF0000">'+pingfeng[x]+'</font>'+'\t'+directors[x]+'</a></body></html><br />'
	else:
		str+= '<html><body><a href="'+links[x]+'" target="_blank">'+movie[x]+'\t'+'<font color="#00FF00">'+pingfeng[x]+'</font>'+'\t'+directors[x]+'</a></body></html><br />'

with open('movie_data.txt','r',encoding = 'utf-8') as movie_data:
	data = movie_data.read()

	if data == str:
		print('movie no updata')
	else:
		with open('movie_data.txt','w',encoding = 'utf-8') as movie_data:
			movie_data.write(str)
			usr.sendemail.send_('电影更新','以下是最近更新的电影目录：<br />'+str)
		print('send email ok')
